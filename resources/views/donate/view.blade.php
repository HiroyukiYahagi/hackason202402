...
<div id="donate-form" class="uk-margin-medium">
  <form name="donateForm" method="post" action="{{route('donate.view', ['hash' => $hash])}}">
    ...
    <div class="uk-margin">
      <label class="uk-form-label" for="form-stacked-text">申請者名<span class="uk-label uk-label-danger uk-margin-small-left">必須</span></label>
    ...
    <div class="uk-margin" v-if="!showPrice">
      ...
      <div class="uk-margin-small uk-text-right">
        <a class="uk-button uk-button-link" v-on:click="showPrice = true;">
          任意の金額を寄附する
        </a>
      </div>
    </div>
...
let vm = new Vue( {
  ...
  data: {
    plan: "m",
    plans: [
      { tag: "s", price: 1980, cta:"2週間のごはん代" },
      { tag: "m", price: 3980, cta:"1ヶ月分のごはん代", is_recommend: true },
      { tag: "l", price: 19800, cta:"ワクチン接種費用" },
    ],
    price: 3980,
    showPrice: false,
    name: "",
  ...
  watch: {
    plan: function(n, o){
      this.price = this.plans.filter( function(p){
        return p.tag == n
      })[0].price
    },
    price: function(){
  ...
  methods: {
    ...
    catSubmit: function(){
      return this.price > 0 && this.card_number.length > 14 && this.card_cvc.length > 2 && this.name.length > 2
    }
  }
});
</script>
